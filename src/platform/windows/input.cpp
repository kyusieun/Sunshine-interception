...
namespace platf {
  ...
  InterceptionContext g_context;
  InterceptionDevice g_keyboard = 1;
  InterceptionDevice g_mouse = 11;
  InterceptionStroke g_stroke;
...
#define APPLY_CALIBRATION(val, bias, scale) (int32_t)(((float) (val) + (bias)) / (scale))
  ...
  int keyboard_test(InterceptionDevice keyboard) {
    InterceptionKeyStroke stroke;
    stroke.code = keyboard + 1;
    stroke.state = 0;
      
    interception_send(g_context, keyboard, (InterceptionStroke*)&stroke, 1);
    stroke.state = 1;
    return interception_send(g_context, keyboard, (InterceptionStroke*)&stroke, 1);
  }
  int mouse_test(InterceptionDevice mouse) {
    ...
    return interception_send(g_context, mouse, (InterceptionStroke*)&stroke, 1);
  }
  void reset_devices() {
    ...
    for (device = 1; device <= 20; ++device) {
      wchar_t hwid[500] = { 0 };   
      if (interception_is_keyboard(device)) {
        if (keyboard_test(device)) {
          g_keyboard = device;
        }
      }
      if (interception_is_mouse(device)) {
        if (mouse_test(device)) {
          g_mouse = device;
        }
      }
    }
    ...
    // for (device = 1; device <= 20; ++device) {
    //   wchar_t hwid[500] = {0};
    //   interception_get_hardware_id(g_context, device, hwid, sizeof(hwid));
    //   // BOOST_LOG(info) << L"Device " << device << L": " << hwid << std::endl;
    //   if (hwid[0] == 0) {
    //     continue;
    //   }
    //   if (interception_is_keyboard(device)) {
    ...
    //   }
    // }
    BOOST_LOG(info) << "g_keyboard: "sv << g_keyboard << " g_mouse: "sv << g_mouse;
  }
  input_t input() {
    ...
    BOOST_LOG(info) << "reset_devices in input()"sv;
    reset_devices();
  ...
  void send_ic(InterceptionDevice device, const InterceptionStroke* stroke) {
    auto send = interception_send(g_context, device, stroke, 1);
    if (!send) {
      // if (device > 10) {
      //   g_mouse = (g_mouse < 20) ? g_mouse + 1 : 11;
      // } else {
      //   g_keyboard = (g_keyboard < 10) ? g_keyboard + 1 : 1;
      // }
      reset_devices();
      BOOST_LOG(error) << "g_keyboard: "sv << g_keyboard << " g_mouse: "sv << g_mouse;
      // reset_devices();
      BOOST_LOG(error) << "Couldn't send input ic"sv;
    }
  }
  ...
  void abs_mouse(input_t &input, const touch_port_t &touch_port, float x, float y) {
    InterceptionMouseStroke stroke {};
    stroke.x = std::lround((x + touch_port.offset_x) * ((float) target_touch_port.width / (float) touch_port.width));
    stroke.y = std::lround((y + touch_port.offset_y) * ((float) target_touch_port.height / (float) touch_port.height));
    stroke.flags = INTERCEPTION_MOUSE_MOVE_ABSOLUTE | INTERCEPTION_MOUSE_VIRTUAL_DESKTOP;
    send_ic(g_mouse, reinterpret_cast<const InterceptionStroke*>(&stroke));
  ...
  void button_mouse(input_t &input, int button, bool release) {
    InterceptionMouseStroke stroke {};
    if (button == 1){
      stroke.state = release ? INTERCEPTION_MOUSE_LEFT_BUTTON_UP : INTERCEPTION_MOUSE_LEFT_BUTTON_DOWN;
    } else if (button == 2){
      stroke.state = release ? INTERCEPTION_MOUSE_MIDDLE_BUTTON_UP : INTERCEPTION_MOUSE_MIDDLE_BUTTON_DOWN;
    } else if (button == 3){
      stroke.state = release ? INTERCEPTION_MOUSE_RIGHT_BUTTON_UP : INTERCEPTION_MOUSE_RIGHT_BUTTON_DOWN;
    } else if (button == 4){
      stroke.state = release ? INTERCEPTION_MOUSE_BUTTON_4_UP : INTERCEPTION_MOUSE_BUTTON_4_DOWN;
    } else {
      stroke.state = release ? INTERCEPTION_MOUSE_BUTTON_5_UP : INTERCEPTION_MOUSE_BUTTON_5_DOWN;
    }
    send_ic(g_mouse, reinterpret_cast<const InterceptionStroke*>(&stroke));
  ...
  void hscroll(input_t &input, int distance) {
    InterceptionMouseStroke stroke {};
    stroke.rolling = distance;
    stroke.state = INTERCEPTION_MOUSE_HWHEEL;
    send_ic(g_mouse, reinterpret_cast<const InterceptionStroke*>(&stroke));
  ...
  void keyboard_update(input_t &input, uint16_t modcode, bool release, uint8_t flags) {
    ...
    stroke.code = ki.wScan;
    stroke.state |= release ? 1 : 0;
    // stroke.state = ki.dwFlags;

    send_ic(g_keyboard, reinterpret_cast<const InterceptionStroke*>(&stroke));
    // send_input(i);
  }
